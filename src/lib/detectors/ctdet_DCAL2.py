...
class CtdetDetector(BaseDetector):
    ...
    def process(self, images, return_time=False):
        ...
        for iter in range(iters):#这个是PGD的循环
            ...
            r = 60
            mask = torch.zeros_like(x_adv_temp[0,0,:,:])#(1,3,512,512)
            #print('mask shape',mask.shape)
            loca_adv = torch.zeros_like(hm_adv_sig[:, 0, :, :])
            for cate in range(cates):#这个是类的循环
                loca_adv += hm_adv_sig[:, cate, :, :] > 0.1
            loca_adv_label = (loca_adv).nonzero()
            for index, item in enumerate(loca_adv_label):
                #print(index)
                x1 = item[1] * 4 - int(r/2)
                x2 = item[1] * 4 + int(r/2)
                y1 = item[2] * 4 - int(r/2)
            ...
            noise_tot_ = noise_tot[0].detach().cpu().numpy().transpose(1, 2, 0)
            mask_shape= mask.shape
            #print('mask_shape:{}'.format(noise_tot_))
            mask_ = (mask*255).detach().cpu().numpy()
            #img = img*10
            #cv2.imshow('mask',noise_tot_)
            cv2.imwrite('D:/python/object_detection/CenterNet/outputs/attack/mask.jpg',mask_)
            #cv2.waitKey()
        ...
        x_adv.data = x_adv.mul(mask).data
        noise_tot = noise_tot.mul(mask)
        ################################################################################
        #noise_tot = noise_tot[0].detach().cpu().numpy().transpose(1, 2, 0)
        #img = ((noise * p_std + p_mean) * 255).astype(np.uint8)
        #cv2.imshow('mask', img)
        #cv2.waitKey(0)
        print("DCA_L")
        ...
        #print('L2 norm of pertubation:{} '.format(np.linalg.norm(noise_total[0])))
