...
class MultiPoseDetector(BaseDetector):
    ...
    def debug_noise(self, debugger, images, dets, output, img_name, noise, scale=1):
        ...
        for i in range(1):
            ...
            print(noise)
            noise_1 = ((noise * p_std + p_mean) * 255).astype(np.uint8)
            noise_ = ((noise * 40 * p_std + p_mean)*255).astype(np.uint8)
            p2 = (np.linalg.norm(noise_1[0]) + np.linalg.norm(noise_1[1]) + np.linalg.norm(noise_1[2]))/(3 * 512 * 512)
            print('L2 norm of pertubation:{} '.format(p2))
