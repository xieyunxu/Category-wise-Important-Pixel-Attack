...
class opts(object):
  def __init__(self):
    ...
    self.parser.add_argument('--load_model_test', default='',
                             help='path to pretrained model for black box exp')
    ...
    # log vis_thresh is in the ctdet show_results
    ...
    self.parser.add_argument('--arch_test', default='dla_34',
                             help='model architecture. Currently tested'
                                  'res_18 | res_101 | resdcn_18 | resdcn_101 |'
                                  'dlav0_34 | dla_34 | hourglass')
    self.parser.add_argument('--head_conv_test', type=int, default=-1,
                             help='conv layer channels for output head'
                                  '0 for no conv layer'
                                  '-1 for default setting: '
                                  '64 for resnets and 256 for dla.')
    ...
    self.parser.add_argument('--keep_res', action='store_true',
                             help='keep the original resolution'
                                  ' during validation.')
    self.parser.add_argument('--bbtest', action='store_true',
                             help='black box exp of category-wise attack.')
  ...
  def parse(self, args=''):
    ...
    print('#################',opt.bbtest)
    if opt.bbtest == True: # black box experiment
        opt.head_conv_test = 256 if 'dla' in opt.arch_test else 64
        opt.pad_test = 127 if 'hourglass' in opt.arch_test else 31
        opt.num_stacks_test = 2 if opt.arch_test == 'hourglass' else 1
