...
class Debugger(object):
  ...
  def add_blend_img(self, back, fore, img_id='blend', trans=0.7):
    ...
    if self.theme == 'white':
      fore = 255 - fore
          #fore back是前景后景的意思
    print('fore shape:{}, back shape:{}'.format(fore.shape,back.shape))
  ...
  def gen_colormap(self, img, output_res=None):
      #for example (80,128,128)
    img = img.copy()
    c, h, w = img.shape[0], img.shape[1], img.shape[2]#c=80,h,w=128
    if output_res is None:#还原比例*4
      output_res = (h * self.down_ratio, w * self.down_ratio)
    #img (128,128,80)->(128,128,80,1)
    ...
    for j, e in enumerate(self.edges):
      if points[e].min() > 0:
        cv2.line(self.imgs[img_id], (points[e[0], 0], points[e[0], 1]),
                      (points[e[1], 0], points[e[1], 1]), self.ec[j], 1,
    ...
    if not self.ipynb:
      ...
      for i, (k, v) in enumerate(self.imgs.items()):
        ...
        if len(v.shape) == 3:
          ...
          self.plt.imshow(cv2.cvtColor(v, cv2.COLORMAP_HOT))
        else:
          self.plt.imshow(v)
      self.plt.show()
